...
class TestONNXRuntime(onnx_test_common._TestONNXRuntime):
    ...
    def test_mixed_optional_default_tensor(self):
        class Model(torch.nn.Module):
            def forward(
                self,
                x,
                y: Tensor | None = torch.ones(2, 3),  # noqa: B008
                z: Tensor | None = torch.zeros(2, 3),  # noqa: B008
            ):
                if y is not None:
                    return x + y
                if z is not None:
    ...
    def test_all_optional_default_tensor(self):
        class Model(torch.nn.Module):
            def forward(
                self,
                x: Tensor | None = torch.ones(2, 3),  # noqa: B008
                y: Tensor | None = torch.zeros(2, 3),  # noqa: B008
            ):
                if x is not None:
                    return x
                elif y is not None:
