...
def _build_signature(schema: OpSchema) -> str:
    """Build a signature.
...
def _build_py(schemas: list[OpSchema]):
    ...
    for schema in (pbar := tqdm.tqdm(schemas)):
        ...
        signatures.append(_build_signature(schema))
    return (
...
def _build_py_impl(schemas: list[OpSchema]):
    ...
    for schema in (pbar := tqdm.tqdm(schemas)):
        pbar.set_postfix_str(f"{schema.name}-{schema.since_version}")
        signatures.append(_build_impl_scaffold(schema))
    return (
        "from __future__ import annotations\n\n"
        + "import torch\n"
        + "import torch.fx\n\n\n"
...
def main():
    ...
    impl_file = _build_py_impl(dataclass_schemas)
    output_dir = pathlib.Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / "__init__.py", "w") as f:
        f.write(init_file)
    with open(output_dir / "_impl.py", "w") as f:
        f.write(impl_file)
